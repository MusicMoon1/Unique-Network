...
export const createUserWithEmailAndPasswordAndProfile = async (email, password, userData, images) => {
    try {
...
const addProfileImage = async (user, userData, images) => {
    try {
        ...
        const data = { ...userData, profilePhoto: { name: profileName, url: profileDownloadURL }, coverPhoto: { name: coverName, url: coverDownloadURL } }
...
async function createCollection(sdk, address, itemName, description, ipfs, data, setGetNFTs, getNFTs) {
...
export const createNewNft = (userData, media, setGetNFTs, getNFTs, mnemonic) => {
...
const addNFTsImage = async (userData, media, setGetNFTs, getNFTs, mnemonic) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const soundDownloadURLs = await Promise.all(soundFiles.map(({ ref }) => getDownloadURL(ref)));
            ...
            for (let i = 0; i < soundDownloadURLs.length; i++) {
                let sound = soundDownloadURLs[i] ? { name: userData.id + "-" + media[`sound${i + 1}`].name, url: soundDownloadURLs[i] } : null;
                if (sound) {
                    sounds.push(sound);
                }
            }
            ...
            formData.append(
                "pinataMetadata",
                JSON.stringify({ name: file.name, keyvalues: { itemName, description, price, size, style } })
            );
            ...
            await createCollection(sdk, address, itemName, description, ipfs, data, setGetNFTs, getNFTs);
...
const updateUserProfileImage = async (user, userData, images) => {
    ...
    try {
        ...
        const data = { ...userData, profilePhoto: { name: profileName, url: profileDownloadURL }, }
        updateProfile(data)
